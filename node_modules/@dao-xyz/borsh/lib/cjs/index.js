...
exports.getDiscriminator = exports.validate = exports.serializer = exports.field = exports.variant = exports.getSchemasBottomUp = exports.getSchema = exports.getAllDependencies = exports.getDependencies = exports.deserializeStruct = exports.deserialize = exports.serialize = void 0;
 ...
 * @returns bytes
 */
function serialize(obj, writer = new binary_js_1.BinaryWriter()) {
...
function recursiveSerialize(obj, writer = new binary_js_1.BinaryWriter()) {
...
function serializeStruct(ctor, allowCustomSerializer = true) {
    ...
    while (true) {
        ...
        else if (once && !(0, exports.getDependencies)(ctor, i)?.length) {
            return handle;
        }
        i++;
...
const clearDeserializeStructHandle = (clazz, offset, fromBuffer) => delete clazz.prototype[PROTOTYPE_DESERIALIZATION_HANDLER_OFFSET + offset + (fromBuffer ? MAX_PROTOTYPE_SEARCH : 0)];
const createDeserializeStructHandle = (currClazz, offset, fromBuffer) => {
    ...
    let dependencies = (0, exports.getAllDependencies)(currClazz, offset);
    if (dependencies) {
...
const setDependencyToProtoType = (ctor, offset) => {
    ...
    while (proto.prototype?.constructor != undefined) { // TODO break early if already done this!
        ...
        let dependencies = (0, exports.getDependencies)(proto, newOffset);
        if (dependencies) {
...
const getAllDependencies = (ctor, offset) => {
    let existing = (0, exports.getDependencies)(ctor, offset);
    if (existing) {
        let ret = new Map();
        for (const v of existing) {
            ...
            if (schema.fields.length > 0 || schema.variant != undefined) { // non trivial
                ...
                let req = (0, exports.getAllDependencies)(v, offset);
...
const getDependenciesRecursively = (ctor, offset, mem = []) => {
    let dep = (0, exports.getDependencies)(ctor, offset);
    if (dep) {
        for (const f of dep) {
            if (mem.includes(f)) {
                continue;
...
const getSubMostSchema = (ctor) => {
    ...
    for (var i = 0; i < MAX_PROTOTYPE_SEARCH; i++) {
        ...
        if (!curr && last && !(0, exports.getDependencies)(ctor, i)?.length) {
            return last;
        }
        last = curr;
    }
...
const getSchemasBottomUp = (ctor) => {
    ...
    for (var i = 0; i < 1000; i++) {
        ...
        if (!curr) {
            if (last && !(0, exports.getDependencies)(ctor, i)?.length) {
                return ret;
            }
        }
        else {
...
const validateIterator = (clazzes, allowUndefined, visited) => {
    ...
    clazzes.forEach((clazz, ix) => {
        ...
        (0, exports.getAllDependencies)(clazz, (0, types_js_1.getOffset)(clazz))?.forEach((dependency, key) => {
            if (!lastVariant)
        ...
        schemas.forEach((structSchema, clazz) => {
            structSchema.fields.forEach((field) => {
                ...
                if (field.type instanceof Function) {
                    if (!(0, exports.getSchema)(field.type) && !(0, exports.getAllDependencies)(field.type, (0, types_js_1.getOffset)(clazz))?.size) {
