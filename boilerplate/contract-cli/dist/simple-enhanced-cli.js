...
export class SimpleEnhancedCLI {
    ...
    async runEnhancedCLI(providers, rli) {
    ...
    async deployOrJoin(providers, rli) {
        if (!this.contractInfo) {
            throw new Error('Contract info not available');
        }
        ...
        if (process.env.AUTO_DEPLOY === 'true') {
            ...
            if (deployMode === 'join') {
                ...
                this.contract = await api.deploy(providers, { itemPrice: new Number(1) });
...
Which would you like to do? `;
    ...
    async run(config, logger, dockerEnv) {
...
export const runEnhanced = async (config, logger, dockerEnv) => {
...
//# sourceMappingURL=simple-enhanced-cli.js.map